...
trait StuartWebHooks extends Directives with Json4sSupport with StrictLogging {
  ...
  lazy val healthcheck = path("healthcheck") {
    get {
  ...
  lazy val webhooks = path("webhooks"){
    post{
        ...
        case Some(key) if key == server.authentication.key =>
          entity(as[StuartGenericEvent]) {stuartGenericEvent =>
            ...
            stuartEvent.data match {
              case StuartEventData.Empty => complete(HttpResponse(StatusCodes.BadRequest, entity = serialization.write(stuartEvent)))
              case job: JobEvent =>
